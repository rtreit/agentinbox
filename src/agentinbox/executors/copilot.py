...
class CopilotExecutor(Executor):
    ...
    def execute(self, ctx: ExecutionContext) -> ExecutionResult:
        ...
        prompt = (
            ...
            f"{ctx.instruction}\n\n"
            f"After completing the task, write your final response "
            f"(a concise summary for GroupMe, max 900 chars) to:\n"
            f"  {reply_file}\n\n"
            f"Do NOT include markdown formatting in the reply file — "
            f"it will be posted as plain text to GroupMe."
        )
        prompt_file.write_text(prompt, encoding="utf-8")

        # Build the command
        ps_command = (
            f"$p = Get-Content -Raw '{_powershell_literal(str(prompt_file))}'; "
        ...
        try:
            ...
            if not reply_text:
                reply_text = "Done — task completed."
